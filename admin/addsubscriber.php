...
$fullname = $firstname.' '.$lastname;
			$qry1 = "SELECT phone FROM subscribers WHERE phone='" . addslashes($phone) . "'";
			if (mysql_num_rows(mysql_query($qry1)) > 0) {
			$err .= "* Phone number already exists!<br>";
			}

			$qry2 = "SELECT username FROM subscribers WHERE username='" . addslashes($username) . "' ";
			...
			if (strlen($err) > 1){
				...
				$sql = mysql_query("INSERT INTO subscribers(fullname,username,pwd,level,phone,email) VALUES ('$fullname','$username','$password','$priv','$phone','$email')");
if ($sql){
